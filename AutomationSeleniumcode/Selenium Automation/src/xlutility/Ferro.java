...
public class Ferro {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		driver.get("http://in-blr-natools:5300/IKON2_EUC/login?accountName=FER");
		
		driver.findElement(By.cssSelector("input[name='username']")).sendKeys("balsunda");
		driver.findElement(By.cssSelector("input[name='password']")).sendKeys("balsunda@123");
		...
		driver. findElement(By.id("nav-tickets-tab")).click();
		 WebElement ele= driver.findElement(By.name("searchType"));
		   ...
		   Select s=new Select(ele);
			s.selectByValue("2");
			// Thread.sleep(5000);
			 WebElement ele1= driver.findElement(By.xpath("//th[@rowspan='1'][11]"));
				
				Actions a=new Actions(driver);
				a.doubleClick(ele1).perform();
		
				String path=".\\datafiles\\HealthCheck_V2_08-06-2023.xlsx";
...
//		int rows=table.findElements(By.xpath("tr")).size();
		//String abc[]= {"",""};
		String reportedDate = driver.findElement(By.xpath("//table[@class='table table-header table-hover border f13 table-responsive dataTable no-footer']/tbody/tr[1]/td[11]")).getText();
		
		 
		
		xlutil.setCellData("Sheet1", 13, 2, reportedDate);
		
		String lastupdatedate = driver.findElement(By.xpath("//p[@class='p-0 pb-1 mb-1 cg-dark-blue font-weight-bold']")).getText();
		xlutil.setCellData("Sheet1", 13, 3, lastupdatedate);
		...
		WebElement ele3= driver.findElement(By.xpath("//th[@rowspan='1'][10]"));
		
		Actions a1=new Actions(driver);
		a1.doubleClick(ele3).perform();
		
        String lastkocreated = driver.findElement(By.xpath("//table[@class='table table-header table-hover border f13 table-responsive dataTable no-footer']/tbody/tr[1]/td[10]")).getText();
		
		 
		
		xlutil.setCellData("Sheet1", 13, 4, lastkocreated);
		...
		xlutil.setCellData("Sheet1", 13, 6, mttr);
        TakesScreenshot ts3= (TakesScreenshot) driver;
		
		File ram3=ts3.getScreenshotAs(OutputType.FILE);
		
		File dest3=new File("./photo/dashboardferro.png");
